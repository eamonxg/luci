...
function collectSections(includes, results, layout) {
	...
	for (let i = 0; i < includes.length; i++) {
		...
		// Items carry the id of the widget they belong to. Includes that do
		// not declare widgets keep working: their items are always shown.
		for (const slot of widgets.slots)
			(content[slot] || (slot == 'cards' ? content.kpi : null) || []).forEach(item => {
				if (item instanceof Node)
					...
					found[slot][item.id] = (slot == 'tabs') ? item : item.node;
			});
	}
